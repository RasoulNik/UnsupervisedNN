...
save_model=1
P_over_noise=120; # dB
train_mode = 1 # set this value to 1 for training the model, otherwise it loads the pretrained model
# cost_type='maxmin'
...
if train_mode:
   ...
   xin = np.reshape(np.log(SNR), [SNR.shape[0], -1])
   # xin = tf.linalg.diag_part(SNR)
   Xin_av = np.mean(xin, axis=0)
   Xin_std = np.std(xin,axis=0)
   unn.Xin_av = Xin_av
   unn.Xin_std = Xin_std
   #-------------------------
   xin = (xin - unn.Xin_av) / unn.Xin_std
   cost,_,_ = unn(xin,SNR)
   current_dir = os.getcwd()
   path = os.path.join(current_dir, 'models_trained', cost_type+'UNN.mod')
   load_model(unn,path)
