...
class Data(Layer):
    ...
    #        Xin.append([reshape([x,y],[1,Nuser*2])])
            ...
            x=np.random.randint(0,self.grid_size, size=[self.Nap,1])
            y=np.random.randint(0,self.grid_size, size=[self.Nap,1])
            z=self.Zap+np.zeros([self.Nap,1],dtype='float32')  
            Xap=np.concatenate((x*self.EX/self.grid_size,y*self.EY/self.grid_size,z),axis=1)
    ...
    #        d=Dist_no_wrap(Xap,Xuser,EX,EY)
            ...
            if Status:
                ...
                # ind_i=np.tile([i],self.Nap)
                Xin[i,x,y,:]=np.expand_dims(g_linear,axis=1)
                i=i+1 
        G=G.astype('float64')
